######################################################################
#
# OpenACS.org Site Configuration for NaviServer
#
# NOTE:
# This file is a *snapshot-style example* derived from the OpenACS.org
# configuration. It is provided for documentation and illustration
# purposes only.
#
# It does NOT necessarily reflect the current production configuration
# of openacs.org, and it omits site-specific, private, or experimental
# settings that are not part of a generally reusable setup.
#
# Use this file as a template and starting point, not as a drop-in
# replacement for a live system.
#
######################################################################
...
#
# ----------------------------------------------------------------------
# openacs.org instance-specific overrides
# ----------------------------------------------------------------------
# These values still can be overridden via environment variables
# prefixed with "oacs_" (e.g. oacs_httpport, oacs_db_host, ...).
#

# Main server name and pretty name
dict set defaultConfig server           "openacs.org"
dict set defaultConfig serverprettyname "OpenACS Web Site"

# File system layout
dict set defaultConfig serverroot       /var/www/openacs.org
dict set defaultConfig logdir           /var/www/openacs.org/log

# Hostnames for the main OpenACS server
dict set defaultConfig hostname {
    openacs.org
    www.openacs.org
    www.openacs.net
    openacs.net
    ...
    openacs.com
    openacs.wu.ac.at
    openacs.wu-wien.ac.at
    smtp.openacs.org
}

# IP addresses and ports. For the migration phase, we keep 8000/8443,
# so the container can coexist with the bare-metal nsd on 80/443.
# Later, just change these to 80/443 (and adjust -b on final startup
# if you don't use host networking).
dict set defaultConfig ipaddress {137.208.116.31 2001:628:404:74::31}
dict set defaultConfig httpport  8000
dict set defaultConfig httpsport 8443
dict set defaultConfig loopbackport 8888

# Database configuration (same as the current openacs.org config)
dict set defaultConfig db_name     "openacs.org"
dict set defaultConfig db_user     "openacs"
dict set defaultConfig db_port     "5413"

# With network_mode=host, localhost still points to the host PG
dict set defaultConfig db_host     "localhost"

dict set defaultConfig smtpdport   "2525"
dict set defaultConfig smtprelay   "localhost:25"

dict set defaultConfig max_file_upload_size     120MB
...
ns_section ns/parameters {
    ...
    ns_param systemlog    error.log         ;# default: nsd.log
    ...
    # ns_sendmail; superseded by nssmtpd module.
...
if {[info exists httpport] && $httpport ne ""} {
    ...
    ns_section ns/module/http {
        ...
        ns_param writersize         4kB    ;# default: 1MB; use writer threads for responses larger than this size
...
if {[info exists loopbackport] && $loopbackport ne ""} {
    ns_section ns/modules {
        ns_param loopback nssock
    }

    ns_section ns/module/loopback {
        ns_param	defaultserver	$server
        ns_param	address		0.0.0.0 ;# #127.0.0.1
        ns_param	port		$loopbackport
    }
}
...
# HTTP/3 driver (quic.so), piggybacking on HTTPS config
if {$httpsport ne ""} {
    ns_section ns/modules {
        #ns_param h3 quic.so
    }
    ...
    ns_section ns/module/h3 {
        ...
        # future H3-specific parameters go here
    }
}
...
ns_section ns/server/$server {
    ...
    # Retry-After header for 503 responses (if rejectoverrun is true).
    ...
    #
    ns_param maxthreads        40        ;# default: 10; max number of connection threads
...
ns_section "ns/server/$server/pool/bots" {
    ns_param   map "GET  /* {user-agent *ot*}"
...
#
# Configure loopback access log for ${server}
#
if {[info exists loopbackport] && $loopbackport ne ""} {
    ns_section ns/server/$server/modules {
        ns_param nslog-loopback nslog
    }
    ns_section ns/server/$server/module/nslog-loopback {
        ns_param driver "loopback"
        ns_param file    access-loopback.log
        ns_param rollfmt %Y-%m-%d       ;# format appended to log filename
        ns_param logpartialtimes true	;# default: false, include high-res start time
        #                                # and partial request durations (accept, queue, filter, run)
        ns_param logthreadname	 true	;# default: false, include thread name for linking with system log
    }
}
...
ns_section "ns/server/$server/module/nssmtpd" {
    ...
    ns_param address     127.0.0.1             ;# local interface SMTP server
...
ns_section ns/server/$server/module/letsencrypt {
    ...
    ns_param domains {
        openacs.org www.openacs.org
        openacs.net www.openacs.net
        openacs.wu.ac.at openacs.wu-wien.ac.at
        smtp.openacs.org
        dotlrn.org dotlrn.net dotlrn.com
        fisheye.openacs.org cvs.openacs.org
        xotcl.org www.xotcl.org
    }
}
...
ns_section ns/server/$server/acs/acs-mail-lite {
    ...
    if {$smtpdport ne ""} {ns_param SMTPPort $smtpdport}
}
...
#=====================================================================
# Section 6.2 – Server "cvs.openacs.org"
#=====================================================================

ns_section "ns/servers" {
    ns_param cvs "CVS redirector"
}
ns_section ns/module/http/servers {
...
ns_section ns/server/cvs/fastpath {
    # just for obtaining the acme challenge
    ns_param pagedir ${pageroot}
}
ns_section ns/server/cvs/tcl {
    ns_param initcmds {
        ns_register_proc GET /* {ns_returnredirect https://openacs.org/github-activities}
        ns_register_proc POST /* {ns_returnredirect https://openacs.org/github-activities}
        # just for obtaining the acme challenge
...
#=====================================================================
# Section 6.3 – Server "xotcl.org"
#=====================================================================
ns_section "ns/servers" {
    ns_param xotcl "XOTcl Web Site"
}
ns_section ns/module/http/servers {
...
ns_section ns/server/xotcl/fastpath {
    # just for obtaining the ACME challenge
    ns_param pagedir ${pageroot}
}

#=====================================================================
# Section 6.4 – Server "dotlrn.org"
#=====================================================================
ns_section ns/servers {
        ns_param dotlrn			"DotLRN Web Site"
}
ns_section ns/server/dotlrn {
        ns_param	minthreads	5	;# 1; minimal number of connection threads
        ns_param	compressenable	on	;# false, use "ns_conn compress" to override
        ns_param        enabletclpages  true    ;# default: false
        ns_param	serverdir	$homedir
}
ns_section ns/server/dotlrn/fastpath {
        ns_param	pagedir		$pageroot/../www-dotlrn
...
ns_section ns/server/dotlrn/modules {
        ns_param        nslog       nslog.so
}

ns_section ns/server/dotlrn/module/nslog {
        ns_param	file			access-dotlrn.log
        ns_param	logpartialtimes	true	;# false, include high-res start time and partial request durations (accept, queue, filter, run)
        ns_param	logthreadname	true	;# false, include thread name for linking with error.log
}
